...
!  isonb(m) :: flag if node is on the boundary, see below for values           !
!                                                                              !
...
!----------------------------REPORT--------------------------------------------!
   IF(MSR)WRITE(IPT,*  )'!'
   IF(MSR)WRITE(IPT,*)'!           SETTING UP TRIS/ELEMENTS/EDGES/CVS          '
   IF(MSR)WRITE(IPT,*  )'!'
!----------------------------INITIALIZE----------------------------------------!
...
!
   ...
   DO I=1,NT 
     ...
     END IF
   END DO
   IF(MSR)WRITE(IPT,*)  '!  ISONB SETTING         :    COMPLETE'
...
!
   ...
   DO I=1,M
     IF(ISONB(I) == 0) THEN
       ...
       IF(NBVE(I,1) /= NBVE(I,NTMP)) THEN
          PRINT*, MYID,I,'NBVE(I) NOT CORRECT!!'
          CALL PSTOP
       ENDIF
   ...
   IF(MX_NBR_ELEM+3 -MAXVAL(NTSN) < 0)THEN
      ...
      CALL PSTOP
   END IF
   IF(MSR)WRITE(IPT,*)  '!  NBVE/NBVT             :    COMPLETE'
...
#  else
   ...
   END DO
#  endif
   IF(MSR)WRITE(IPT,*)  '!  EDGE SETUP            :    COMPLETE'
...
#  if defined (MULTIPROCESSOR)
       PRINT*,'SORRY, THE BOUNDARY CELL',NGID(I),'IS NOT GOOD FOR MODEL.'
#  else
...
!
   ...
   IF(MSR)WRITE(IPT,*)  '!  NISBCE/LISBCE/EPOR    :    COMPLETE'

   IF(MSR)WRITE(IPT,*)  '!  TRIS/EDGES/CVOLS      :    COMPLETE'
