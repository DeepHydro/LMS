...
!==============================================================================|
   use lims,         only: kbm1
   use control,      only: igotm,inpdir,casename,msr,ipt
...
!------------------------------------------------------------------------------!
   ...
   FNAME = "./"//TRIM(INPDIR)//"/"//trim(casename)//'_gotmturb.inp'
...
!--Variables for Interfacing with GOTM-----------------------
   ...
   DOUBLE PRECISION          :: P(0:KBM1),B(0:KBM1)
...
!      if(z0s_gotm < z0s_min) z0s_gotm = z0s_min
      ...
      DO J=1,MaxItz0b
        z0b_gotm = 0.1*umol/max(umold,u_taub)+0.03*(z0b/.03)
...
!        u_taub = rr*sqrt( u(i,kbm1)*u(i,kbm1) + v(i,kbm1)*v(i,kbm1) )
      ...
      P             =  num*SS1d        !Shear Production              [m^2/s^3]
      B             = -nuh*NN1d        !Buoyancy Production           [m^2/s^3]
      ...
      call do_turbulence(kbm1,time_step,depth,u_taus,u_taub,z0s_gotm,z0b_gotm,h,nn1D,ss1D,p,b) 
