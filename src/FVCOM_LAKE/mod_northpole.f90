...
!
#  if !defined (LIMITED_NO)
   ALLOCATE(UIJ1(NPE),VIJ1(NPE),UIJ2(NPE),VIJ2(NPE))
   ALLOCATE(UALFA(0:NT),VALFA(0:NT))
   ALLOCATE(FXX(NPE),FYY(NPE))
...
!     DLTYC_TMP = VY2_TMP-VY1_TMP
     ...
     IF(CELL_NORTHAREA(IA) == 1 .AND. CELL_NORTHAREA(IB) == 1)THEN
      PSTX(IA)=PSTX(IA)-GRAV_E(IA)*D1(IA)*ELIJ*DLTYC_TMP/(2._SP /(1._SP+sin(YC(IA)*PI/180.0_SP)))
      PSTY(IA)=PSTY(IA)+GRAV_E(IA)*D1(IA)*ELIJ*DLTXC_TMP/(2._SP /(1._SP+sin(YC(IA)*PI/180.0_SP)))
      PSTX(IB)=PSTX(IB)+GRAV_E(IB)*D1(IB)*ELIJ*DLTYC_TMP/(2._SP /(1._SP+sin(YC(IB)*PI/180.0_SP)))
      PSTY(IB)=PSTY(IB)-GRAV_E(IB)*D1(IB)*ELIJ*DLTXC_TMP/(2._SP /(1._SP+sin(YC(IB)*PI/180.0_SP)))
     ELSE IF(CELL_NORTHAREA(IA) == 1 .AND. CELL_NORTHAREA(IB) /= 1)THEN
      PSTX(IA)=PSTX(IA)-GRAV_E(IA)*D1(IA)*ELIJ*DLTYC_TMP/(2._SP /(1._SP+sin(YC(IA)*PI/180.0_SP)))
      PSTY(IA)=PSTY(IA)+GRAV_E(IA)*D1(IA)*ELIJ*DLTXC_TMP/(2._SP /(1._SP+sin(YC(IA)*PI/180.0_SP)))
     ELSE IF(CELL_NORTHAREA(IB) == 1 .AND. CELL_NORTHAREA(IA) /= 1)THEN  
...
!==============================================================================!
   ...
   SUBROUTINE ADV_UV_EDGE_XY(XFLUX,YFLUX,CETA)
