...
!----Write Data to File------------------------------------------------------
     ...
     IF(Probe_Mode =='Original') THEN
         CALL PROBE_STORE(I)
         WRITE(IUNIT,*)THOUR,(PROBE(I)%TMP(K),K=K1,K2)
    ELSE
          DLOC  = PROBE(I)%D_LOC
        !THOUR EL UV_A  T_AVRG U(K) V(K) W(K) WW(K) T(K)
         UV_AVRG=SQRT(UA(DLOC)*UA(DLOC)+VA(DLOC)*VA(DLOC))
         T_AVRG= SUM(T1(DLOC,K1:K2))/ (K2-K1+1)
         WRITE(IUNIT,'(30E14.4)')THOUR, EL(DLOC),UV_AVRG,T_AVRG, &
         (U(DLOC,K),V(DLOC,K),W(DLOC,K),WW(DLOC,K),K=K1,K2),(T(DLOC,K), K=K1,K2)
     ENDIF
