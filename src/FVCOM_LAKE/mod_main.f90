...
MODULE CONTROL
   ...
   CHARACTER(LEN=80) DotFVM_VERSION !!STRING DESCRIBING VERSION
   CHARACTER(LEN=80) DotFVM_WEBSITE !!STRING DESCRIBING WEBSITE FOR DOTFVM INFO 
...
!--File Unit Specifiers (see iofiles.f for details)----------------------------!
   INTEGER  INRUN,INGRD,INCWH,INRIV,INITS, &
            INJUL,INHFX,INWND,INEVP,INELF,INUVF,INLAG, &
...
!---------------- internal mode   arrays-(element based)----------------------------!
   ...
   REAL(SP), ALLOCATABLE :: WW(:,:)        !Z-VELOCITYA
...
CHARACTER(LEN=80) InitialWQ_Mode  !Uniform or Node
CHARACTER(LEN=80) SMS_OUT_Mode  !SIMPLE or COMPLETE
CHARACTER(LEN=80) Probe_Mode  !Original or Custom
CHARACTER(LEN=80) Language 
REAL(SP) ::  BaseSurfaceElevation
LOGICAL :: SMS_AVGE_ON
LOGICAL :: SMS_ON = .true.
LOGICAL:: MEDM_ON
LOGICAL:: Output_ON = .true.
INTEGER:: IOLOG
REAL(SP) :: StefanBoltz, Kelvin
REAL(SP) :: R2D
