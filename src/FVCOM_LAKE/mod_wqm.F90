...
!   INTEGER  :: iurun1,iuprt1,iuprt2
   ...
   !----------------------------My Variables--------------------------------------! 
    REAL(SP) :: K_DO_REA  !!Global Reaeration Rate Constant
    REAL(SP) :: DO_Saturation  !!Global DO Saturation
    REAL(SP) :: K_death !! Phytoplankton death rate
    REAL(SP), ALLOCATABLE :: WQ_Default(:)
     REAL(SP), ALLOCATABLE :: WQ_Max(:)
...
!   and rewriten by J. Qi for DOTFVM in 2002 and 2004.			      !
!=============================================================================!
...
!---------------------------------------------------------------------------
   ...
   INTEGER::K
   ALLOCATE(WQ_Default(8)); WQ_Default  = ZERO
    ALLOCATE(WQ_Max(8)); WQ_Max  = ZERO
!   //ISTR = "./"//TRIM(INPDIR)//"/"
      ISTR =TRIM(INPDIR)//"/"
...
! skip head line
  ...
  ! skip head line
   ...
   READ(31,*) HEADLINE
   READ(31,*) K_DO_REA
   READ(31,*) DO_Saturation
   READ(31,*) K_death
   
   ! skip head line
   READ(31,*) HEADLINE
   READ(31,*) (WQ_Default(K), K=1,NB)

   ! skip head line
   READ(31,*) HEADLINE
   READ(31,*) (WQ_MAX(K), K=1,NB)
...
!-----------------------------------------------------------------------------!
   ...
   IF( InitialWQ_Mode .EQ. 'Uniform') THEN
   DO I=1,8
      READ(1,*) HEADLINE 
   ENDDO
   READ(1,*) WQM1 
   ...
   ELSE
    DO I=1,M
        READ(1,*) WQM1,WQM2,WQM3,WQM4,WQM5,WQM6,WQM7,WQM8
           WQM(I,1:KBM1,1) = WQM1
           WQM(I,1:KBM1,2) = WQM2
           WQM(I,1:KBM1,3) = WQM3
           WQM(I,1:KBM1,4) = WQM4
           ...
           WQM(I,1:KBM1,8) = WQM8
            ...
            WQM_T(I,1:KBM1,1) = WQM1
            WQM_T(I,1:KBM1,2) = WQM2
            WQM_T(I,1:KBM1,3) = WQM3
            WQM_T(I,1:KBM1,4) = WQM4
            WQM_T(I,1:KBM1,5) = WQM5
            WQM_T(I,1:KBM1,6) = WQM6
            WQM_T(I,1:KBM1,7) = WQM7
            WQM_T(I,1:KBM1,8) = WQM8

            WMEAN(I,1:KBM1,1) = WQM1
            WMEAN(I,1:KBM1,2) = WQM2
            WMEAN(I,1:KBM1,3) = WQM3
            WMEAN(I,1:KBM1,4) = WQM4
            WMEAN(I,1:KBM1,5) = WQM5
            WMEAN(I,1:KBM1,6) = WQM6
            WMEAN(I,1:KBM1,7) = WQM7
            WMEAN(I,1:KBM1,8) = WQM8
     ENDDO
   ENDIF
   CLOSE(1)
...
#      endif
         DO K = 1, KBM1
#        if defined (MPDATA)     
           !WQM_F(I,K,N1)=(WQM(I,K,N1)-XFLUX(I,K,N1)/ART1(I)*(DTI/DT(I)))*  (DT(I)/D(I))+RF(I,K,N1)*DTI
             
           WQM_F(I,K,N1)=(WQM(I,K,N1)-XFLUX(I,K,N1)/ART1(I)*(DTI/(DT(I)*DZ(I,K))))*  (DT(I)/DTFA(I))+RF(I,K,N1)*DTI
                  !WQM_F(I,K,N1)=(WQM(I,K,N1)-XFLUX(I,K,N1)/ART1(I)*(DTI/(DT(I)*DZ(I,K))))* (DT(I)/DTFA(I))
#        else
           WQM_F(I,K,N1)=(WQM_SF(I,K) -XFLUX(I,K,N1)/ART1(I)*DTI/DTFA(I)) +RF(I,K,N1)*DTI
#    endif              
         END DO
#      if defined (WET_DRY)
       ELSE
   ...
   DO I = 1, M
     DO K = 1, KBM1
       IF(D(I) > 0.0_SP .AND. K == 1) THEN
         ...
         U_W = U_W/NTVE(I)
         V_W = V_W/NTVE(I)
         T_W = T1(I,K)         ! JQI In Zheng''s code 
         !T_W=20.0
