...
!------------------------------------------------------------------------------!
   ...
   IF(SERIAL)THEN
     ...
     WRITE(1) N,M,KB,IINT,IINT-INT_AVGE
     DO I=1,N
...
#  if defined (MULTIPROCESSOR)
   IF(PAR)THEN
     ...
     ALLOCATE(KMTMP(MGL,KB))
     ALLOCATE(KHTMP(MGL,KB))
     CALL GATHER(LBOUND(U,1), UBOUND(U,1), N,NGL,KB,MYID,NPROCS,EMAP,U_AVE,  UTMP)
     CALL GATHER(LBOUND(V,1), UBOUND(V,1), N,NGL,KB,MYID,NPROCS,EMAP,V_AVE,  VTMP)
     CALL GATHER(LBOUND(WW,1),UBOUND(WW,1),N,NGL,KB,MYID,NPROCS,EMAP,W_AVE,  WWTMP)
     CALL GATHER(LBOUND(KM,1),UBOUND(KM,1),M,MGL,KB,MYID,NPROCS,NMAP,KM_AVE, KMTMP)
     CALL GATHER(LBOUND(KH,1),UBOUND(KH,1),M,MGL,KB,MYID,NPROCS,NMAP,KH_AVE, KHTMP)
     IF(MSR)THEN
       WRITE(1) IINT,NGL,MGL,THOUR
       DO I=1,NGL
     ...
     CALL GATHER(LBOUND(EL,1),  UBOUND(EL,1),  M,MGL, 1,MYID,NPROCS,NMAP,EL_AVE,ELTMP)
     CALL GATHER(LBOUND(T1,1),  UBOUND(T1,1),  M,MGL,KB,MYID,NPROCS,NMAP,T_AVE, T1TMP)
     CALL GATHER(LBOUND(S1,1),  UBOUND(S1,1),  M,MGL,KB,MYID,NPROCS,NMAP,S_AVE, S1TMP)
     CALL GATHER(LBOUND(RHO1,1),UBOUND(RHO1,1),M,MGL,KB,MYID,NPROCS,NMAP,R_AVE, R1TMP)
     IF(MSR)THEN
       DO I=1,MGL
...
#  if defined (BALANCE_2D) 
     ...
     ALLOCATE(ADVUA2TMP(NGL))
     ALLOCATE(ADVVA2TMP(NGL))
     ALLOCATE(ADFX2TMP(NGL))
     ALLOCATE(ADFY2TMP(NGL))
     ALLOCATE(DRX2D2TMP(NGL))
     ALLOCATE(DRY2D2TMP(NGL))
     ALLOCATE(CORX2TMP(NGL))
     ALLOCATE(CORY2TMP(NGL))
     ALLOCATE(PSTX2TMP(NGL))
     ALLOCATE(PSTY2TMP(NGL))
     ALLOCATE(ADX2D2TMP(NGL))
     ALLOCATE(ADY2D2TMP(NGL))
     ALLOCATE(WUSURBF2TMP(NGL))
     ALLOCATE(WVSURBF2TMP(NGL))
     ALLOCATE(DUDT2TMP(NGL))
     ALLOCATE(DVDT2TMP(NGL))
     CALL GATHER(LBOUND(ADVUA2_AVE,1),  UBOUND(ADVUA2_AVE,1),   N,NGL,1,MYID,NPROCS,EMAP,ADVUA2_AVE,   ADVUA2TMP)
     CALL GATHER(LBOUND(ADVVA2_AVE,1),  UBOUND(ADVVA2_AVE,1),   N,NGL,1,MYID,NPROCS,EMAP,ADVVA2_AVE,   ADVVA2TMP)
     CALL GATHER(LBOUND(ADFX2_AVE,1),   UBOUND(ADFX2_AVE,1),    N,NGL,1,MYID,NPROCS,EMAP,ADFX2_AVE,    ADFX2TMP)
     CALL GATHER(LBOUND(ADFY2_AVE,1),   UBOUND(ADFY2_AVE,1),    N,NGL,1,MYID,NPROCS,EMAP,ADFY2_AVE,    ADFY2TMP)
     CALL GATHER(LBOUND(DRX2D2_AVE,1),  UBOUND(DRX2D2_AVE,1),   N,NGL,1,MYID,NPROCS,EMAP,DRX2D2_AVE,   DRX2D2TMP)
     CALL GATHER(LBOUND(DRY2D2_AVE,1),  UBOUND(DRY2D2_AVE,1),   N,NGL,1,MYID,NPROCS,EMAP,DRY2D2_AVE,   DRY2D2TMP)
     CALL GATHER(LBOUND(CORX2_AVE,1),   UBOUND(CORX2_AVE,1),    N,NGL,1,MYID,NPROCS,EMAP,CORX2_AVE,    CORX2TMP)
     CALL GATHER(LBOUND(CORY2_AVE,1),   UBOUND(CORY2_AVE,1),    N,NGL,1,MYID,NPROCS,EMAP,CORY2_AVE,    CORY2TMP)
     CALL GATHER(LBOUND(PSTX2_AVE,1),   UBOUND(PSTX2_AVE,1),    N,NGL,1,MYID,NPROCS,EMAP,PSTX2_AVE,    PSTX2TMP)
     CALL GATHER(LBOUND(PSTY2_AVE,1),   UBOUND(PSTY2_AVE,1),    N,NGL,1,MYID,NPROCS,EMAP,PSTY2_AVE,    PSTY2TMP)
     CALL GATHER(LBOUND(ADX2D2_AVE,1),  UBOUND(ADX2D2_AVE,1),   N,NGL,1,MYID,NPROCS,EMAP,ADX2D2_AVE,   ADX2D2TMP)
     CALL GATHER(LBOUND(ADY2D2_AVE,1),  UBOUND(ADY2D2_AVE,1),   N,NGL,1,MYID,NPROCS,EMAP,ADY2D2_AVE,   ADY2D2TMP)
     CALL GATHER(LBOUND(WUSURBF2_AVE,1),UBOUND(WUSURBF2_AVE,1), N,NGL,1,MYID,NPROCS,EMAP,WUSURBF2_AVE, WUSURBF2TMP)
     CALL GATHER(LBOUND(WVSURBF2_AVE,1),UBOUND(WVSURBF2_AVE,1), N,NGL,1,MYID,NPROCS,EMAP,WVSURBF2_AVE, WVSURBF2TMP)
     CALL GATHER(LBOUND(DUDT2_AVE,1),   UBOUND(DUDT2_AVE,1),    N,NGL,1,MYID,NPROCS,EMAP,DUDT2_AVE,    DUDT2TMP)
     CALL GATHER(LBOUND(DVDT2_AVE,1),   UBOUND(DVDT2_AVE,1),    N,NGL,1,MYID,NPROCS,EMAP,DVDT2_AVE,    DVDT2TMP)
     IF(MSR)THEN
       DO I=1,NGL
