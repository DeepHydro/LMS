...
! FREE SURFACE (EL) -e
       ...
       IKEYS(NCNT) = 101
       FLIMS(1,NCNT) = MINVAL(EL(1:M))
       FLIMS(2,NCNT) = MAXVAL(EL(1:M))
! UA -u
       NCNT = NCNT + 1
       FKEYS(NCNT) = 1
       TKEYS(NCNT) = "UA"
       TKEYS(NCNT) = "UA"
       IKEYS(NCNT) = 117
       FLIMS(1,NCNT) = MINVAL(UA(1:N))
       FLIMS(2,NCNT) = MAXVAL(UA(1:N))
! VA -v
       NCNT = NCNT + 1
       FKEYS(NCNT) = 1
       TKEYS(NCNT) = "VA"
       IKEYS(NCNT) = 118
       FLIMS(1,NCNT) = MINVAL(VA(1:N))
       FLIMS(2,NCNT) = MAXVAL(VA(1:N))
! DEPTH -b
       NCNT = NCNT + 1
       FKEYS(NCNT) = 1
       TKEYS(NCNT) = "BATHYMETRIC DEPTH"
       IKEYS(NCNT) = 98
       FLIMS(1,NCNT) = MINVAL(H(1:M))
       FLIMS(2,NCNT) = MAXVAL(H(1:M))
! SALINITY S
       NCNT = NCNT + 1
       FKEYS(NCNT) = 1
       TKEYS(NCNT) = "SALINITY"
       IKEYS(NCNT) = 83
       FLIMS(1,NCNT) = MINVAL(S1(1:M,1:KBM1))
       FLIMS(2,NCNT) = MAXVAL(S1(1:M,1:KBM1))
! TEMPERATURE T
       NCNT = NCNT + 1
       FKEYS(NCNT) = 1
       TKEYS(NCNT) = "TEMPERATURE"
       IKEYS(NCNT) = 84
       FLIMS(1,NCNT) = MINVAL(T1(1:M,1:KBM1))
       FLIMS(2,NCNT) = MAXVAL(T1(1:M,1:KBM1))
! X-VELOCITY U
       NCNT = NCNT + 1
       FKEYS(NCNT) = 1
       TKEYS(NCNT) = "U"
       IKEYS(NCNT) = 85
       FLIMS(1,NCNT) = MINVAL(U(1:N,1:KBM1))
       FLIMS(2,NCNT) = MAXVAL(U(1:N,1:KBM1))
! Y-VELOCITY V
       NCNT = NCNT + 1
       FKEYS(NCNT) = 1
       TKEYS(NCNT) = "V"
       IKEYS(NCNT) = 86
       FLIMS(1,NCNT) = MINVAL(V(1:N,1:KBM1))
       FLIMS(2,NCNT) = MAXVAL(V(1:N,1:KBM1))
! TURB LENGTH SCALE L
       NCNT = NCNT + 1
       FKEYS(NCNT) = 1
       TKEYS(NCNT) = "TURBULENT LENGTH"
       IKEYS(NCNT) = 76
       FLIMS(1,NCNT) = MINVAL(L(1:N,1:KBM1))
       FLIMS(2,NCNT) = MAXVAL(L(1:N,1:KBM1))
! TURB LENGTH SCALE Q
       NCNT = NCNT + 1
       FKEYS(NCNT) = 1
       TKEYS(NCNT) = "TURBULENT KE"
       IKEYS(NCNT) = 81
       FLIMS(1,NCNT) = MINVAL(Q2(1:N,1:KBM1))
       FLIMS(2,NCNT) = MAXVAL(Q2(1:N,1:KBM1))
! WTSURF h
       NCNT = NCNT + 1
       FKEYS(NCNT) = 1
...
! VERTICAL VELOCITY W
       ...
       IKEYS(NCNT) = 87
       FLIMS(1,NCNT) = MINVAL(WW(1:N,1:KBM1))
       FLIMS(2,NCNT) = MAXVAL(WW(1:N,1:KBM1))
! WET/DRY               
       NCNT = NCNT + 1
       FKEYS(NCNT) = 1
       TKEYS(NCNT) = "WET/DRY"
...
!      DEPTH (D) -d
       ...
       TKEYS(NCNT) = "DEPTH"
       IKEYS(NCNT) = 100
       FLIMS(1,NCNT) = MINVAL(D(1:M))
       FLIMS(2,NCNT) = MAXVAL(D(1:M))
...
!-------------------------------------------------------------------------------
      ...
      DO NNCNT=1,N
        NCC = NCC + 1
        CEL3(1,NCC) = NV(NNCNT,1)+(K-1)*M
        CEL3(2,NCC) = NV(NNCNT,1)+(K*M)
        CEL3(3,NCC) = NV(NNCNT,2)+(K*M)
        CEL3(4,NCC) = NV(NNCNT,2)+(K-1)*M
        CEL3(5,NCC) = NV(NNCNT,3)+(K*M)
        CEL3(6,NCC) = NV(NNCNT,3)+(K-1)*M
        END DO
      END DO
      RETURN
...
!
      DO NNCNT=1,N
        NCC = NCC + 1
        SCEL(1,NCC) = NV(NNCNT,1) + M*(KBM1-1)
        SCEL(2,NCC) = NV(NNCNT,2) + M*(KBM1-1)
        SCEL(3,NCC) = NV(NNCNT,3) + M*(KBM1-1)
        SCEL(4,NCC) = 0
      END DO
      NSURF(1,2) = NCC
      NSURF(2,2) = 0
