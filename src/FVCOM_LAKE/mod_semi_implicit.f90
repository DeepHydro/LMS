...
# endif   
# if defined (WET_DRY)
  USE MOD_WD, ONLY : WET_DRY_ON, ISWETC, ISWETCT, WET_JUDGE
# endif
  ...
  REAL(SP) :: XFLUX(0:MT), XFLUX_TMP(0:NT), YFLUX_TMP(0:NT), XFLUX_TMP_NP(0:NT), YFLUX_TMP_NP(0:NT)
...
#   endif
  ENDDO
# if defined (MULTIPROCESSOR)
  IF(PAR) CALL EXCHANGE(EC,NT,1,MYID,NPROCS,XFLUX_TMP,YFLUX_TMP)
# endif
...
# endif
# if defined (MULTIPROCESSOR)
  IF(PAR) CALL EXCHANGE(EC,NT,1,MYID,NPROCS,XFLUX_TMP_NP,YFLUX_TMP_NP)
# endif
...
# if defined (WET_DRY)
  IF(WET_DRY_ON) THEN
    ELF = EL
    CALL WET_JUDGE
    IF(PAR) CALL EXCHANGE(NC,MT,1,MYID,NPROCS,ELF)
    EL = ELF
  ENDIF
# endif
...
# endif   
  ...
  REAL(SP) Y3Y2,Y1Y3,Y2Y1,X3X2,X1X3,X2X1
  REAL(SP) :: XFLUX(0:MT), XFLUX_TMP(0:NT), YFLUX_TMP(0:NT)
  REAL(SP) :: UF_AVG(0:NT), VF_AVG(0:NT)
  ...
  VF_AVG = 0.0_SP
# if defined(MULTIPROCESSOR)
   IF(PAR) CALL EXCHANGE(EC,NT,KB,MYID,NPROCS,UF,VF)
# endif
  DO I=1,NT
